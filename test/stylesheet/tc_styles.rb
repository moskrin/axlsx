...
class TestStyles < Test::Unit::TestCase
  ...
  def test_add_style_border_hash
    ...
    edges = @styles.borders.last.prs.map { |pr| pr.name }
    [:top,:bottom,:left,:right].each {|edge| assert(edges.include?(edge))}
    ...
    edges = @styles.borders.last.prs.map { |pr| pr.name }
    assert(edges.include?(:top))
    assert(edges.include?(:bottom))
    assert(! edges.include?(:left))
    assert(! edges.include?(:right))
  end
